...
- 可在“小空间”绑定 Stock Platform 联动手机号；两端手机号摘要匹配后，接收每个交易日 16:00 的股票观察清单提醒。
- 微信小程序通过 `wx.login` 获取 code，由服务端调用微信接口换取 OpenID 并签发自己的会话；正式代码不提供体验账号。
- Android App 内测版使用账号和密码登录；微信小程序老用户可在“小空间”给原账号补设 App 凭据，新用户才走注册。密码通过随机盐和 `scrypt` 摘要保存，App 内支持退出和永久注销账号。
...
- Android/H5 本地书架支持从系统文件选择器导入自己创作、已获授权或公版的 TXT 文件，兼容 UTF-8、GBK 编码，自动识别中文章节、保存阅读进度，并可调整字号、行距和阅读背景。书籍正文不上传服务器，也不内置或抓取来源和授权不明的第三方书源。
...
现有多项目 ECS 使用 Nginx 时，默认只把应用映射到 `127.0.0.1:8787`，Caddy 被放入可选的 `standalone` profile，不会占用宿主机的 80/443。Nginx 配置见 `deploy/nginx-notes.conf`，当前允许最大 32 MB 请求体，以承载运营台上传的 20 MB TXT 及 JSON 编码开销；自动部署会校验并同步该限制。独立服务器需要 Caddy 时执行 `docker compose --profile standalone --env-file deploy/.env up -d`。
