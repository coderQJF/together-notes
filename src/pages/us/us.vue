...
const appVersion = import.meta.env.VITE_APP_VERSION || '开发版'
...
function addHomeWidget() {
  if (isHomeWidgetPinSupported() && requestHomeWidgetPin()) {
    uni.showToast({ title: '请在系统窗口确认添加', icon: 'none' })
    return
  }
  uni.showModal({
    title: '添加桌面小工具',
...
<template>
  <view class="shell">
    ...
    <template v-else>
      <view class="headline-row"><view class="headline"><text>我们的</text><text>小空间。</text></view><text class="version">v{{ appVersion }}</text></view>
      ...
      <view class="identity-row">
        ...
        <button v-if="!editingNickname" class="edit-nickname" aria-label="编辑我的昵称" @click="beginNicknameEdit"><image src="/static/nav-icons/edit-active.png" mode="aspectFit" /><text>编辑</text></button>
      </view>
