...
const DEFAULT_COMPETITIONS: CompetitionId[] = ['epl', 'laliga', 'ucl', 'lol']
...
<template>
  <view class="shell">
    ...
    <view class="mode-tabs">
      <button hover-class="none" :class="{ active: mode === 'upcoming' }" :aria-pressed="mode === 'upcoming'" @click="mode = 'upcoming'"><text>近期赛程</text></button>
      <button hover-class="none" :class="{ active: mode === 'results' }" :aria-pressed="mode === 'results'" @click="mode = 'results'"><text>历史赛果</text></button>
      <button hover-class="none" :class="{ active: mode === 'standings' }" :aria-pressed="mode === 'standings'" @click="mode = 'standings'"><text>积分榜</text></button>
    </view>
    ...
    <view v-else-if="payload" class="standings-card">
      <view v-if="payload.standingsWarning" class="inline-warning"><text>{{ payload.standingsWarning.message }}</text><text>{{ payload.standingsWarning.code }}</text></view>
...
.mode-tabs{display:flex;align-items:stretch;gap:4px;padding:4px;border:1px solid #e5dece;border-radius:16px;background:#fff}.mode-tabs button{display:flex;align-items:center;justify-content:center;height:46px;min-height:46px;min-width:0;flex:1;margin:0;padding:0 7px;border:0;border-radius:12px;background:transparent;color:#6f6555;font-size:13px;line-height:normal;box-sizing:border-box}.mode-tabs button text{display:block;line-height:20px}.mode-tabs button::after{border:0}.mode-tabs button.active{background:#494032;color:#fff9e9}.mode-tabs button:active{opacity:.86}.section-heading{display:flex;align-items:center;justify-content:space-between;margin:24px 2px 10px}.section-title{font-size:19px;font-weight:600}.section-count{color:#786d5b;font-size:12px}
